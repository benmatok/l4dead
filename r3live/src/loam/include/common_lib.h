...
public:
    ...
    static std::string to_string(const StatesGroup &state, std::string str = std::string(""))
    {
        ...
        snprintf(state_str,2000, "%.8f,", state.last_update_time);
        msg.append(state_str);
        snprintf(state_str,2000,"%.8f,%.8f,%.8f,", angle_axis(0), angle_axis(1), angle_axis(2));
        msg.append(state_str);
        snprintf(state_str,2000, "%.8f,%.8f,%.8f,", state.pos_end(0), state.pos_end(1), state.pos_end(2));
        msg.append(state_str);
        snprintf(state_str,2000, "%.8f,%.8f,%.8f,", state.vel_end(0), state.vel_end(1), state.vel_end(2));
        msg.append(state_str);
        snprintf(state_str,2000, "%.8f,%.8f,%.8f,", state.bias_g(0), state.bias_g(1), state.bias_g(2));
        msg.append(state_str);
        snprintf(state_str,2000, "%.8f,%.8f,%.8f,", state.bias_a(0), state.bias_a(1), state.bias_a(2));
        msg.append(state_str);
        snprintf(state_str,2000, "%.8f,%.8f,%.8f", state.gravity(0), state.gravity(1), state.gravity(2));
        msg.append(state_str);
        return msg;
    }
